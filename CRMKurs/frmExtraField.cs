...
namespace CRMKurs
{
    public partial class frmExtraField : Form
    {
        ...
        private void frmExtraField_Load(object sender, EventArgs e)
        {
            ...
            var types = (from x in DBConnection.DbCon.PersonTypes select x.Name).ToArray();
        ...
        {
            PersonTypes pType = (from x in DBConnection.DbCon.PersonTypes where x.Name==cbAlan.SelectedItem.ToString() select x).First();
            if (pType == null)
            {
                return;
            }
